function handleRemovedRelationshipRequest(evt) {
            ...
            if(errors.hasOwnProperty(key)) {
                if(Array.isArray(errors[key])) {
                    errors[key].forEach(message => {
                        const alertDiv = createErrorMessage(message, 'alert-danger');
    ...
    } else if (evt.detail.xhr.status == 200 || evt.detail.xhr.status == 204) {
        location.reload();
    }
}
...
function createErrorMessage(message, messageClass) {
    ...
    const messageHTML = `
    <div class="alert ${messageClass} alert-dismissible fade show" role="alert" id="${idString}">
        <div>${message}</div>
